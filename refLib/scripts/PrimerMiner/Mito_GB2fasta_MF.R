Mito_GB2fasta <- function(files, marker=c("COX1", "COXi", "CO1", "COi","cox I","CO I","Cytochrome oxidase subunit I","cytochrome c oxidase subunit I"), add=100, rm_dup=T, pattern="\\.gb$", no_marker=T, setwd=NULL){
...
spec_name <- gsub("^.*\"(.*)\\\"$", "\\1", temp[grep("/organism=", temp)])

sequence_exp <- c(sequence_exp, paste(">", paste(sequ_name,spec_name, sep=" "), "\n", sep=""), paste(sequ, "\n", sep=""))} else { # export mito with no marker
