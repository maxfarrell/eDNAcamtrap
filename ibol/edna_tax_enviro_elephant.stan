...
data {
    ...
    vector[N_ID] temp;                  // mean temperature (celsius) at time of sampling
    ...
    vector[N_ID] ele36;                 // number of elephant observations within previous 36hours
...
parameters {
    ...
    real beta_temp;                     // coef for temp
    real beta_mS;                       // coef for mS
    real beta_DO;                       // coef for DO
    real beta_pH;                       // coef for pH
    real beta_ele36;                    // coef for ele36
...
transformed parameters {
    ...
    ID_eff =  beta_temp * temp
            + beta_mS * mS
            + beta_DO * DO
            + beta_pH * pH
            + beta_ele36 * ele36
            + sigma_site * site_raw[site]
            + sigma_ID * ID_raw; 
...
model {
    ...
    beta_temp ~ cauchy(0,2.5);     
    beta_mS ~ cauchy(0,2.5);     
    beta_DO ~ cauchy(0,2.5);     
    beta_pH ~ cauchy(0,2.5);     
    beta_ele36 ~ cauchy(0,2.5);     
