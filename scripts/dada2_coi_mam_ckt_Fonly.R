...
# Gibson 2015 (mam_ckt) designed to amplify 310bp fragment
...
out
saveRDS(out, "mam_ckt_filterAndTrim_Fonly_out.rds")

sum(out[,1])-sum(out[,2])
(sum(out[,1])-sum(out[,2]))/sum(out[,1])
# 2,179 sequences (19.6%) lost 
...
learn_derep_save <- function(filt_Fs, Batch.sample.names, output){
	...
	saveRDS(ddFs, paste("Batch",length(Batch.sample.names),"ddFs","mam_ckt","Fonly.rds",sep="_"))
}

learn_derep_save(Batch12.filtFs, Batch12.sample.names, "./Batch12_seqtab_mam_ckt_Fonly.rds")
learn_derep_save(Batch15.filtFs, Batch15.sample.names, "./Batch15_seqtab_mam_ckt_Fonly.rds")
learn_derep_save(Batch54.filtFs, Batch54.sample.names, "./Batch54_seqtab_mam_ckt_Fonly.rds")
learn_derep_save(Batch7.filtFs, Batch7.sample.names, "./Batch7_seqtab_mam_ckt_Fonly.rds")
...
# Merge multiple runs 
st1 <- readRDS("./Batch12_seqtab_mam_ckt_Fonly.rds")
st2 <- readRDS("./Batch15_seqtab_mam_ckt_Fonly.rds")
st3 <- readRDS("./Batch54_seqtab_mam_ckt_Fonly.rds")
st4 <- readRDS("./Batch7_seqtab_mam_ckt_Fonly.rds")
...
# Save final seqtab
saveRDS(seqtab.nochim.pooled, "../results/coi_mam_ckt_Fonly_seqtab.rds")


## Track reads
# reading back in "out"
out <- readRDS("mam_ckt_filterAndTrim_Fonly_out.rds")
rownames(out) <- sample.names

# Bringing in dada and merger read counts for tracking
ddFs1 <- readRDS("./Batch_12_ddFs_mam_ckt_Fonly.rds")
ddFs2 <- readRDS("./Batch_15_ddFs_mam_ckt_Fonly.rds")
ddFs3 <- readRDS("./Batch_54_ddFs_mam_ckt_Fonly.rds")
ddFs4 <- readRDS("./Batch_7_ddFs_mam_ckt_Fonly.rds")
...
require(xtable)
print(xtable(track, digits=0),file="../plots_tables/coi_mam_ckt_Fonly_track_reads.tex")
...
print(xtable(track_cam, digits=0),file="../plots_tables/coi_mam_ckt_Fonly_track_reads_cam_samples.tex")
colSums(track_cam)
# input 	6161
# filtered 	4799
# denoised	1805
