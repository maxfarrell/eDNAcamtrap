...
path <- "/data/kruger/illumina/coi_BR5_fastq" # directory containing the fastq files
list.files(path)
...
# Gibson 2015 (BR5) designed to amplify 310bp fragment
# Should have lots of overlap for merging

pdf("../plots_tables/BR5_quality_profiles.pdf")
...
# Gibson 2015 (BR5) designed to amplify 310bp fragment
# Lets keep forward to 230 and reverse to 220 bp reverse
...
out
saveRDS(out, "BR5_filterAndTrim_out.rds")

sum(out[,1])-sum(out[,2])
(sum(out[,1])-sum(out[,2]))/sum(out[,1])
# 264,933 sequences (11.4%) lost with truncLen=c(200,150) & maxEE=c(4,4) & truncQ=6
...
learn_derep_merge_save <- function(filt_Fs, filt_Rs, Batch.sample.names, output){
	...
	saveRDS(ddFs, paste("Batch",length(Batch.sample.names),"ddFs","BR5.rds",sep="_"))
	saveRDS(mergers, paste("Batch",length(Batch.sample.names),"mergers","BR5.rds",sep="_"))

}

learn_derep_merge_save(Batch12.filtFs, Batch12.filtRs, Batch12.sample.names, "./Batch12_seqtab_BR5.rds")
learn_derep_merge_save(Batch15.filtFs, Batch15.filtRs, Batch15.sample.names, "./Batch15_seqtab_BR5.rds")
learn_derep_merge_save(Batch54.filtFs, Batch54.filtRs, Batch54.sample.names, "./Batch54_seqtab_BR5.rds")
learn_derep_merge_save(Batch7.filtFs, Batch7.filtRs, Batch7.sample.names, "./Batch7_seqtab_BR5.rds")
...
# Merge multiple runs 
st1 <- readRDS("./Batch12_seqtab_BR5.rds")
st2 <- readRDS("./Batch15_seqtab_BR5.rds")
st3 <- readRDS("./Batch54_seqtab_BR5.rds")
st4 <- readRDS("./Batch7_seqtab_BR5.rds")
...
# Write to disk
saveRDS(seqtab.nochim.pooled, "./coi_BR5_seqtab.rds")
...
# reading back in "out"
out <- readRDS("BR5_filterAndTrim_out.rds")
rownames(out) <- sample.names

# Bringing in dada and merger read counts for tracking
ddFs1 <- readRDS("./Batch_12_ddFs_BR5.rds")
ddFs2 <- readRDS("./Batch_15_ddFs_BR5.rds")
ddFs3 <- readRDS("./Batch_54_ddFs_BR5.rds")
ddFs4 <- readRDS("./Batch_7_ddFs_BR5.rds")
ddFs <- c(ddFs1, ddFs2, ddFs3, ddFs4)

mergers1 <- readRDS("./Batch_12_mergers_BR5.rds")
mergers2 <- readRDS("./Batch_15_mergers_BR5.rds")
mergers3 <- readRDS("./Batch_54_mergers_BR5.rds")
mergers4 <- readRDS("./Batch_7_mergers_BR5.rds")
...
require(xtable)
print(xtable(track, digits=0),file="../plots_tables/coi_BR5_track_reads.tex")
...
print(xtable(track_cam, digits=0),file="../plots_tables/coi_BR5_track_reads_cam_samples.tex")
colSums(track_cam)
# input 	1387895
# filtered 	1231536
# denoised	1206823
