...
path <- "/data/kruger/illumina/coi_mam_ckt_fastq" # directory containing the fastq files
list.files(path)
...
# 0% 18, 25% 251, 50% 251, 75% 251, 100% 301
...
# 0% 20, 25% 230, 50% 232, 75% 232, 100% 283
# hist(as.numeric(unlist(lapply(reverse,nchar))))

sum(as.numeric(unlist(lapply(reverse,nchar)))<50)# 1
sum(as.numeric(unlist(lapply(reverse,nchar)))<100)# 9
sum(as.numeric(unlist(lapply(reverse,nchar)))<200)# 14
sum(as.numeric(unlist(lapply(reverse,nchar)))<230)# 971

# Mean length is 500
mean(as.numeric(unlist(lapply(forward,nchar))))# 257.82
mean(as.numeric(unlist(lapply(reverse,nchar))))# 242.05
258 + 242

# Unsure what length of sequence the mini-barcode primers are targeting...

pdf("../plots_tables/mam_ckt_quality_profiles.pdf")
plotQualityProfile(fnFs, aggregate=TRUE) 
plotQualityProfile(fnRs, aggregate=TRUE) 
dev.off()

# Forward quality is very odd (getts better with read length...)
# Reverse drops quite quickly - maybe not worth including?

# Try trimleft on forward reads to about 90?
# Trim right to 230?
# Reverse trim to 125?
...
# for now, no length truncation or restriction by maxEE
# only remove N bases (dada2 requirement)
out <- filterAndTrim(fnFs, filtFs, fnRs, filtRs,
				truncLen=c(230, 140),
              	maxN=0, truncQ=6, rm.phix=TRUE, maxEE=c(4,4),
              	compress=TRUE, multithread=3) # On Windows set multithread=FALSE
out
saveRDS(out, "mam_ckt_filterAndTrim_out.rds")

sum(out[,1])-sum(out[,2])
(sum(out[,1])-sum(out[,2]))/sum(out[,1])
# 2281 sequences (20%) lost with truncLen=c(230,140) & maxEE=c(4,4) & truncQ=6
...
learn_derep_merge_save <- function(filt_Fs, filt_Rs, Batch.sample.names, output){
	...
	saveRDS(ddFs, paste("Batch",length(Batch.sample.names),"ddFs","mam_ckt.rds",sep="_"))
	saveRDS(mergers, paste("Batch",length(Batch.sample.names),"mergers","mam_ckt.rds",sep="_"))

}

learn_derep_merge_save(Batch12.filtFs, Batch12.filtRs, Batch12.sample.names, "./Batch12_seqtab_mam_ckt.rds")
learn_derep_merge_save(Batch15.filtFs, Batch15.filtRs, Batch15.sample.names, "./Batch15_seqtab_mam_ckt.rds")
learn_derep_merge_save(Batch54.filtFs, Batch54.filtRs, Batch54.sample.names, "./Batch54_seqtab_mam_ckt.rds")
learn_derep_merge_save(Batch7.filtFs, Batch7.filtRs, Batch7.sample.names, "./Batch7_seqtab_mam_ckt.rds")
...
# Merge multiple runs 
st1 <- readRDS("./Batch12_seqtab_mam_ckt.rds")
st2 <- readRDS("./Batch15_seqtab_mam_ckt.rds")
st3 <- readRDS("./Batch54_seqtab_mam_ckt.rds")
st4 <- readRDS("./Batch7_seqtab_mam_ckt.rds")
...
# Write to disk
saveRDS(seqtab.nochim.pooled, "../results/coi_mam_ckt_seqtab.rds")
...
# reading back in "out"
out <- readRDS("mam_ckt_filterAndTrim_out.rds")
rownames(out) <- sample.names

# Bringing in dada and merger read counts for tracking
ddFs1 <- readRDS("./Batch_12_ddFs_mam_ckt.rds")
ddFs2 <- readRDS("./Batch_15_ddFs_mam_ckt.rds")
ddFs3 <- readRDS("./Batch_54_ddFs_mam_ckt.rds")
ddFs4 <- readRDS("./Batch_7_ddFs_mam_ckt.rds")
ddFs <- c(ddFs1, ddFs2, ddFs3, ddFs4)

mergers1 <- readRDS("./Batch_12_mergers_mam_ckt.rds")
mergers2 <- readRDS("./Batch_15_mergers_mam_ckt.rds")
mergers3 <- readRDS("./Batch_54_mergers_mam_ckt.rds")
mergers4 <- readRDS("./Batch_7_mergers_mam_ckt.rds")
...
require(xtable)
print(xtable(track, digits=0),file="./coi_mam_ckt_track_reads.tex")
...
print(xtable(track_cam, digits=0),file="../plots_tables/coi_mam_ckt_track_reads_cam_samples.tex")
colSums(track_cam)
# input 	6161
# filtered 	4742
# denoised	1745
...
# Trying with Kruger Mammal specific references
# seqtab <- readRDS("./coi_mam_ckt_seqtab.rds")

# # Kindgom -> Genus assignment
# tax <- assignTaxonomy(seqtab, "./Kruger_Mammals_refLib_dada2.fasta", multithread=TRUE)
# saveRDS(tax, "./coi_mam_ckt_mammals_dada2_genus.rds")

# # add species by exact sequence matching
# # doesn't work with justConcatenate=TRUE
# # tax <- addSpecies(tax, "./Kruger_Mammals_refLib_dada2_species.fasta")
# # saveRDS(tax, "./coi_mam_ckt_mammals_dada2_exactSpecies.rds")

# # Kindgom -> species assignment
# tax <- assignTaxonomy(seqtab, "./Kruger_Mammals_refLib_dada2_phy2species.fasta", multithread=TRUE)
# saveRDS(tax, "./coi_mam_ckt_mammals_dada2_phy2species.rds")

# # Additional potential training datasets for RDP
# # https://github.com/terrimporter/CO1Classifier
# # https://www.nature.com/articles/sdata201727

# # Exporting unique sequences for external taxonomy assignment
# uniquesToFasta(seqtab, "./coi_mam_ckt_seqs.fa")
# # Run external tax assignment on coi_seq.fa